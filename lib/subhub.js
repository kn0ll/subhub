...
Client.prototype.subscribe = function() {
  ...
  data.form['hub.mode'] = 'subscribe';
  data.form['hub.verify'] = 'sync';
  request.post(url, data, callback);
};

Client.prototype.unsubscribe = function() {
  log('requesting pubsub unsubscribe');
  var config = this.config,
    url = config['hub'],
    data = { form: config['subscription'] },
    callback = _.bind(this.handleSubscription, this);

  data.form['hub.mode'] = 'unsubscribe';
  data.form['hub.verify'] = 'sync';
  request.post(url, data, callback);
};
...
Client.prototype.middleware = function() {
  var self = this,
    config = this.config,
    cb = config['subscription']['hub.callback'],
    path = url.parse(cb).path,
    route = new RegExp(path);
  log('creating http handlers for ' + path);
